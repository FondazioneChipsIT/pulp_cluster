...
SYN_PARAM += set LIBRARIES [list ${LIBRARIES}];\
			set MEMCUTS [list ${REGFILECUTS_RVT} ${REGFILECUTS_LVT} ${SRAMCUTS}]; \
			set BENDER_TARGS [list ${common_targs} ${synth_targs}]; \
         	set BENDER_DEFS  [list ${common_defs} ${synth_defs}] 
...
restore_dc_run:
	dcnxt_shell -topographical_mode -x "${SYN_PARAM}; set TOPO 1; source synth_scripts/reload.tcl"

synth_run: 
	dcnxt_shell -x "${SYN_PARAM}; source synth_scripts/pulp_cluster_SY.tcl"

synth_run_topo:
	dcnxt_shell -topographical_mode -x "${SYN_PARAM}; set TOPO 1; source synth_scripts/pulp_cluster_SY.tcl"
...
floorplan_run:
	cd innovus/  && \
	innovus -overwrite -execute "${PNR_PARAM}; source scripts/pulp_cluster/pulp_cluster.tcl"